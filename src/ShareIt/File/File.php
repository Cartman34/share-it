...
class File extends PermanentEntity {
	 ...
	 * @return string
	 */
	public function getLink($download = false) {
		return u(ROUTE_FILE_DOWNLOAD, ['fileID' => $this->id()]) . '?k=' . urlencode($this->passkey) . ($download ? '&download' : '');
	}
	...
	public function getFileName() {
		return strpos($this->name, '.') !== false ? $this->name : toSlug($this->name) . '.' . $this->extension;
	}
	 ...
	 * @throws    Exception
	 */
	public static function uploadOne($inputName, $usage, $label = null, $parent = 0) {
